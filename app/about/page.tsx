          ...
          <div className="mt-14 grid grid-cols-4 gap-9 max-lg:grid-cols-2 max-md:gap-[36px]">
            {aboutContent.team.members.map((member, i) => (
              <article key={i} className="reveal">
                ...
                <span className="block text-[0.68rem] font-extrabold tracking-[0.16em] uppercase text-gold">
                  {member.role}
                </span>
                <p className="mt-3 m-0 text-[0.9rem] font-light text-charcoal">{member.bio}</p>
              </article>
            ))}
          </div>
        </div>
